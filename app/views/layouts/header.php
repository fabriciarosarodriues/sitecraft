...
<header class="site-header">
    <div class="topbar">
        <div class="container topbar__inner">
            ...
            <div class="topbar__actions">
                <?php if ($user): ?>
                    <?php
                    // Mostrar botón de notificaciones para cualquier usuario autenticado.
                    $unreadCount = 0;
                    try {
                        require_once __DIR__ . '/../../controllers/NotificationController.php';
                        $notifController = new NotificationController(db());
                        $notificationStats = $notifController->getStats((int) $user['id']);
                        $unreadCount = (int) ($notificationStats['unreadCount'] ?? 0);
                    } catch (Throwable) {
                        // Mantener botón visible con contador 0 si falla la consulta.
                    }
                    ?>

                    <a href="<?= htmlspecialchars(appUrl('student/notificaciones.php'), ENT_QUOTES, 'UTF-8'); ?>" 
                       class="btn btn-outline-dark btn-sm custom-outline notifications-btn" 
    ...
    <div class="branding-bar">
        <div class="container branding-bar__inner">
            ...
            <form class="search-box" role="search">
                <input type="search" class="form-control" placeholder="Busca tu curso..." aria-label="Buscar cursos">
    ...
    <nav class="main-nav navbar navbar-expand-lg">
        <div class="container">
            <button class="navbar-toggler custom-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#mainNavigation" aria-controls="mainNavigation" aria-expanded="false" aria-label="Mostrar navegación">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="mainNavigation">
                <ul class="navbar-nav mx-auto gap-lg-4">
                    <li class="nav-item"><a class="nav-link <?= ($currentPage ?? '') === 'home' ? 'active' : ''; ?>" href="<?= htmlspecialchars(appUrl('index.php'), ENT_QUOTES, 'UTF-8'); ?>">Inicio</a></li>
                    <li class="nav-item"><a class="nav-link <?= ($currentPage ?? '') === 'courses' ? 'active' : ''; ?>" href="<?= htmlspecialchars(appUrl('cursos.php'), ENT_QUOTES, 'UTF-8'); ?>">Cursos</a></li>
                    <li class="nav-item"><a class="nav-link" href="#como-funciona">Cómo funciona</a></li>
                    <li class="nav-item"><a class="nav-link <?= ($currentPage ?? '') === 'teachers' ? 'active' : ''; ?>" href="<?= htmlspecialchars(appUrl('profesores.php'), ENT_QUOTES, 'UTF-8'); ?>">Profesores</a></li>
                    <li class="nav-item"><a class="nav-link <?= ($currentPage ?? '') === 'help' ? 'active' : ''; ?>" href="<?= htmlspecialchars(appUrl('ayuda.php'), ENT_QUOTES, 'UTF-8'); ?>">Ayuda</a></li>
                    <li class="nav-item"><a class="nav-link" href="<?= htmlspecialchars(appUrl('index.php#contacto'), ENT_QUOTES, 'UTF-8'); ?>">Contacto</a></li>
                </ul>
            </div>
        </div>
    </nav>
...
<!-- Botones trigger ocultos para abrir modales desde JS -->
<button type="button" id="loginModal-trigger" class="d-none" data-bs-toggle="modal" data-bs-target="#loginModal"></button>
<button type="button" id="registerModal-trigger" class="d-none" data-bs-toggle="modal" data-bs-target="#registerModal"></button>
